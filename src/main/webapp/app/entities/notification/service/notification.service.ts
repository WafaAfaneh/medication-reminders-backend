...
type RestOf<T extends INotification | NewNotification> = Omit<T, 'startDate'> & {
...
export class NotificationService {
  ...
  protected convertDateFromClient<T extends INotification | NewNotification | PartialUpdateNotification>(notification: T): RestOf<T> {
    return {
      ...notification,
      startDate: notification.startDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restNotification: RestNotification): INotification {
    return {
      ...restNotification,
      startDate: restNotification.startDate ? dayjs(restNotification.startDate) : undefined,
    };
  }
